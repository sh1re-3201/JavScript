document.getElementById("OK").addEventListener("click"),function() {
    ...
    submitButton.addEventListener("click", function() {
        let choiceFilled = [];
        for (let i = 1; i <= countChoice; i++) {
            let valuePut = document.getElementById("pilihan" + i).value;
            if(!nilai) {
        ...
        finalSubmit.addEventListener("click", function() {
            ...
            let finalMsg = (document.getElementById("container")
                                .textContent = `Perkenalkan nama saya ${name}, saya memiliki ${countChoice} pilihan yaitu ${choiceFilled.join(", "
                                )}. pilihan saya adalah ${selected.value})`)
